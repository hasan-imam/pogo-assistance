...
class GenericSpawnMessageProcessorPogoSAIntegrationTest {
    ...
    @ParameterizedTest
    @MethodSource(value = {"pogoSaPokemonAlertMessages"})
    void process_VcScans100IvMessages_ReturnsExpected(final Message message) {
        ...
        if (channelName.contains("100iv")) {
            ...
            // Unown channel only seems to have one message with non standard format, so it fails parsing
            assertThat(failureMsgWithJumpUrl, pokemonSpawn.getPokedexEntry().getId(), equalTo(201));
        }
    }

    private static Stream<Message> pogoSaPokemonAlertMessages() {
        return jda.getGuildById(SERVER_ID_POGO_SA).getCategories().stream()
                .filter(category -> CATEGORY_IDS_POGO_SA.contains(category.getIdLong()))
