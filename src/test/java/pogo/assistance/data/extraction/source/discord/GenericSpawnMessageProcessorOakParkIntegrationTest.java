...
class GenericSpawnMessageProcessorOakParkIntegrationTest {
    ...
    @ParameterizedTest
    @MethodSource(value = { "OakParkSpawnMessages" })
    void process_OakParkSpawnMessages_ReturnsExpected(final Message message) {
    ...
    private static Stream<Message> OakParkSpawnMessages() {
        return jda.getGuildById(SERVER_ID_OAK_PARK).getCategories().stream()
                .filter(category -> CATEGORY_ID_OAK_PARK_IV_SCANNERS == category.getIdLong())
