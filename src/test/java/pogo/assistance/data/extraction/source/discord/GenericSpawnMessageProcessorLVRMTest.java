...
import static pogo.assistance.bot.di.DiscordEntityConstants.SERVER_ID_LVRM;
...
class GenericSpawnMessageProcessorLVRMTest {
    ...
    @ParameterizedTest
    @MethodSource(value = { "LVRMSpawnMessages" })
    ...
    private static Stream<Message> LVRMSpawnMessages() {
        return jda.getGuildById(SERVER_ID_LVRM).getCategories().stream()
                .filter(category -> CATEGORY_ID_LVRM_IV_HUNTING == category.getIdLong())
