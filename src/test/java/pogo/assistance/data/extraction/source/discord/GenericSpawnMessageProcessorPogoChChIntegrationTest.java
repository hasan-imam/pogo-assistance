...
class GenericSpawnMessageProcessorPogoChChIntegrationTest {
    ...
    @ParameterizedTest
    @MethodSource(value = { "PogoChChSpawnMessages" })
    void process_LVRM_ReturnsExpected(final Message message) {
        ...
        if (channelName.contains("hundred")) {
    ...
    private static Stream<Message> PogoChChSpawnMessages() {
        return jda.getGuildById(SERVER_ID_POGO_CHCH).getTextChannels().stream()
                .filter(textChannel -> CHANNEL_IDS_POGO_CHCH_FEEDS.contains(textChannel.getIdLong()))
                .map(textChannel -> {
                    try {
