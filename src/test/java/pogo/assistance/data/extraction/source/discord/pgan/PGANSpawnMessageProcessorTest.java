...
class PGANSpawnMessageProcessorTest {

    private static JDA jda;

    private static final MessageProcessor<PokemonSpawn> PROCESSOR = new PGANSpawnMessageProcessor();

    @BeforeAll
    static void setUp() throws LoginException, InterruptedException {
        jda = new JDABuilder(AccountType.CLIENT)
                .setToken(DiscordEntityConstants.CRANK_USER_TOKEN)
                .build()
                .awaitReady();
    }
    ...
    @ParameterizedTest
    @MethodSource(value = { "PganSpawnDms" })
    void process_PganSpawnDms_ReturnsExpected(final Message message) {
    ...
    private static Stream<Message> PganSpawnDms() {
        return USER_ID_PGAN_BOTS.stream()
                .map(jda::getUserById)
                .filter(Objects::nonNull)
                .map(botUser -> botUser.openPrivateChannel().complete())
                .flatMap(messageChannel -> MessageStream.lookbackMessageStream(messageChannel).limit(500))
