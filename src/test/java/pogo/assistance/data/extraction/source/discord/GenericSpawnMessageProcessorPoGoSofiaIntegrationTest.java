...
class GenericSpawnMessageProcessorPoGoSofiaIntegrationTest {
    ...
    @ParameterizedTest
    @MethodSource(value = { "PoGoSofiaSpawnFeedMessages" })
    void process_PoGoSofiaSpawnFeedMessages_ReturnsExpected(final Message message) {
        ...
        if (message.getChannel().getName().equals("100iv")) {
            assertThat(failureMsgWithJumpUrl, pokemonSpawn.getIv().get(), equalTo(100.0));
        }
    }

    private static Stream<Message> PoGoSofiaSpawnFeedMessages() {
        return jda.getGuildById(SERVER_ID_POGO_SOFIA).getCategories().stream()
                .filter(category -> category.getIdLong() == CATEGORY_ID_POGO_SOFIA_SCANNER_COORDINATES)
