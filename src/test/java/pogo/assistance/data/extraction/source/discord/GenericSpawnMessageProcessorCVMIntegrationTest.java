...
class GenericSpawnMessageProcessorCVMIntegrationTest {
    ...
    @ParameterizedTest
    @MethodSource(value = { "CVMPoGoSpawnMessages" })
    void process_CVMSpawnMessages_ReturnsExpected(final Message message) {
        ...
        } else if (channelName.contains("90iv")) {
            assertThat(failureMsgWithJumpUrl, pokemonSpawn.getIv().get(), greaterThanOrEqualTo(90.0));
        }
    }

    private static Stream<Message> CVMPoGoSpawnMessages() {
        return jda.getGuildById(SERVER_ID_CVM).getCategories().stream()
                .filter(category -> CATEGORY_IDS_CVM_FEEDS.contains(category.getIdLong()))
