...
import static pogo.assistance.bot.di.DiscordEntityConstants.CATEGORY_ID_TOAST_MAPS_TEST_STUFF;
...
class GenericSpawnMessageProcessorToastMapsIntegrationTest {
    ...
    static void setUp() throws LoginException, InterruptedException {
        jda = new JDABuilder(AccountType.CLIENT)
                .setToken(DiscordEntityConstants.POKE_PETER_USER_TOKEN)
                .build()
                .awaitReady();
    }
    ...
    @ParameterizedTest
    @MethodSource(value = { "ToastMapsSpawnMessages" })
    void process_ToastMapsSpawnMessages_ReturnsExpected(final Message message) {
    ...
    private static Stream<Message> ToastMapsSpawnMessages() {
        return jda.getGuildById(SERVER_ID_TOAST_MAPS).getCategories().stream()
                .map(Category::getTextChannels)
                .flatMap(Collection::stream)
                .filter(textChannel -> SPAWN_CHANNEL_IDS_TOAST_MAPS.contains(textChannel.getIdLong()))
                .map(messageChannel -> {
                    try {
